...
Today we're going to explore some online maps and dashboard. We will continue to use an example of analyzing Airbnb data in Chicago to illustrate how to use these tools effectively.

1.  Interactive maps with `leaflet`
2.  Introduce dashboards with `flexdashboard`
...
# Continue Woking with Airbnb Data

Last time we tried our own on the scraped data from [Inside Airbnb](http://insideairbnb.com/). On the same [this data downloading page](http://insideairbnb.com/get-the-data/), we worked with the `gz` file last time, but there is also a file **"listings.csv".** They are cleaned-up metrics for Airbnb listings prepared for visualization. We will start here this time.

Visualizations of the Chicago Airbnb data using this dataset can be found [here](http://insideairbnb.com/chicago), where you can see maps showing the type of room, activity, availability, and listings per host for Airbnb listings.

We are going to create an interactive dashboard like that in this lab.
...
# Add Interactive graphs using `plotly`
...
# Exercise: Make your own dashboard

For this week's assignment, [**choose a U.S. city**](http://insideairbnb.com/explore) **other than Chicago** and create a flexdashboard that includes one leaflet map and no less than two charts to describe its Airbnb listings.
