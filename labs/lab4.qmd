...
Let's download and read this file as `parks`. Open spaces are polygon data. As usual, check what each column means and the CRS. This data also uses EPSG: 4326. To calculate areas, we need to use `st_transform()` to convert it to the same projection as the sidewalks, EPSG: 2249.
...
As our result is a spatial object, we can also create maps to show the scores in neighborhoods. This type of map, that shades areas in different colors based on data values, is known as choropleth map.
